"""The mitmproxy addon that serves the claimed Codex slot from a local model.

mitmdump loads this file directly, under mitmproxy's own interpreter rather
than the one that started the launcher. The sys.path insert below is what makes
the rest of the `codex-local` package importable in that interpreter.
"""
...
if (
    ...
    # This file is a mitmproxy addon, but it is easy to mistake it for the
    # launcher. Hand direct execution to the real entry point before importing
    # mitmproxy's embedded modules. The package directory has to be on the
    # child's path explicitly: Codex Local is commonly run straight from a checkout
    # rather than installed, so `-m codex_local` alone would not resolve.
    _package_parent = str(Path(__file__).resolve().parents[1])
    _existing_path = os.environ.get("PYTHONPATH", "")
    os.execve(
        sys.executable,
        [sys.executable, "-m", "codex_local", *sys.argv[1:]],
        {
            **os.environ,
            "PYTHONPATH": (
                f"{_package_parent}{os.pathsep}{_existing_path}"
                if _existing_path
...
# mitmdump runs this file under its own interpreter, where the package is not
# installed. Make `codex-local` importable from the checkout it was loaded out of.
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from codex_local.audit import (  # noqa: E402
    AuditRecorder,
    audit_header_bytes,
...
from codex_local.websocket import (  # noqa: E402
    ResponsesWebSocketState,
    SSEEventDecoder,
    decode_json_events,
)
from codex_local.savings import replay_savings_fields  # noqa: E402
...
LOCAL_TRANSIENT_RETRY_ATTEMPTS = 2
...
PREFIX_PREFILL_TIMEOUT_SECONDS = float(
    os.environ.get("CODEX_LOCAL_PREFIX_PREFILL_TIMEOUT", "180")
)
MODEL_RESIDENCY_INTERVAL_SECONDS = float(
    os.environ.get("CODEX_LOCAL_RESIDENCY_INTERVAL", str(12 * 60))
)
...
LOCAL_UPSTREAM_READ_TIMEOUT_SECONDS = float(
    os.environ.get("CODEX_LOCAL_UPSTREAM_READ_TIMEOUT", "600")
)
# Per-turn preflight: verify the local upstream is alive before forwarding a
# real inference, so a dead server does not silently cost a full 60-200s turn.
# Throttled to one cheap GET /models per window to avoid per-request latency.
PREFLIGHT_INTERVAL_SECONDS = float(
    os.environ.get("CODEX_LOCAL_PREFLIGHT_INTERVAL", str(15))
)
PREFLIGHT_TIMEOUT_SECONDS = float(
    os.environ.get("CODEX_LOCAL_PREFLIGHT_TIMEOUT", "3")
)
...
def _usage_from_replay(content_type: str, body: bytes) -> dict[str, Any] | None:
    """Recover the exact terminal usage block from a cached response body."""
    decoded: Any
    if "text/event-stream" in content_type.lower():
        decoded = decode_json_events(SSEEventDecoder().feed(body))
    else:
        try:
            decoded = json.loads(body)
        except (UnicodeDecodeError, json.JSONDecodeError):
            return None
    values = reversed(decoded) if isinstance(decoded, list) else (decoded,)
    for event in values:
...
class CodexLocalInterceptor:
    def __init__(self) -> None:
        self.upstream_url = _required_env("CODEX_LOCAL_UPSTREAM_URL")
        self.model = _required_env("CODEX_LOCAL_MODEL")
        self.local_slot = os.environ.get("CODEX_LOCAL_LOCAL_SLOT") or None
        self.local_slot_auto = os.environ.get(
            "CODEX_LOCAL_LOCAL_SLOT_AUTO", "0"
        ) in {"1", "true", "True"}
        self.local_label = os.environ.get(
            "CODEX_LOCAL_LOCAL_LABEL", "Local · oMLX"
        )
        self.local_server = os.environ.get("CODEX_LOCAL_LOCAL_SERVER", "oMLX")
        self.api_key = os.environ.get("CODEX_LOCAL_UPSTREAM_API_KEY")
        self.auth_header = os.environ.get(
            "CODEX_LOCAL_AUTH_HEADER", "1"
        ) not in {"0", "false", "False"}
        self.status_path = Path(
            _required_env("CODEX_LOCAL_STATUS_PATH")
        ).expanduser()
        self.session_id = _required_env("CODEX_LOCAL_SESSION_ID")
        self.websocket_bridge = os.environ.get(
            "CODEX_LOCAL_WEBSOCKET_BRIDGE", "1"
        ) not in {"0", "false", "False"}
        self.websocket_upstream_override = os.environ.get(
            "CODEX_LOCAL_WEBSOCKET_UPSTREAM_URL"
        )
        self.gpt_56_pro_enabled = os.environ.get(
            "CODEX_LOCAL_GPT56_PRO", "0"
        ) in {"1", "true", "True"}
        # A local task's compaction must remain local because hosted summaries
        # are ciphertext to private models. Route affinity also keeps cloud-task
        # compaction on OpenAI instead of hijacking all maintenance globally.
        self.local_compaction = os.environ.get(
            "CODEX_LOCAL_LOCAL_COMPACTION", "1"
        ) not in {"0", "false", "False"}
        mode = os.environ.get("CODEX_LOCAL_LOOP_GUARD", "guard")
        ...
        self.residency_keepalive_enabled = os.environ.get(
            "CODEX_LOCAL_RESIDENCY_KEEPALIVE", "1"
        ) not in {"0", "false", "False"}
        ...
        audit_dir = os.environ.get("CODEX_LOCAL_CLOUD_AUDIT_DIR")
        self.cloud_audit_include_credentials = os.environ.get(
            "CODEX_LOCAL_CLOUD_AUDIT_CREDENTIALS", "0"
        ) in {"1", "true", "True"}
        self.cloud_audit = (
            AuditRecorder(
                Path(audit_dir),
        ...
        self.code_fingerprint = source_fingerprint(
            [
                ...
                Path(__file__).resolve().parent / "routing.py",
            ]
        )
    ...
    def _new_websocket_state(self) -> ResponsesWebSocketState:
        """Create a per-socket bridge state with a disk spill dir so evicted
        conversation survives retention caps and can be reloaded."""
    ...
    def _preflight_local_upstream(self) -> bool:
        ...
        Runs at most once per PREFLIGHT_INTERVAL_SECONDS so per-request latency
        stays bounded. A dead server is caught here (cheap GET /models) instead
        of after Codex has already paid for a full local inference turn.
        """
        now = time.monotonic()
        if now - self._last_preflight_at < PREFLIGHT_INTERVAL_SECONDS:
            return True
        ok = _upstream_reachable(
            self.upstream_url,
            self.api_key,
            self.auth_header,
    ...
    def request(self, flow: http.HTTPFlow) -> None:
        ...
        flow.metadata["codex_local_request_started"] = time.monotonic()
        flow.metadata["codex_local_original_host"] = original_host
        flow.metadata["codex_local_original_path"] = safe_path
        flow.metadata["codex_local_original_full_path"] = original_path[:4096]
        flow.metadata["codex_local_local_routed_at"] = time.monotonic()
        flow.metadata["codex_local_original_method"] = request.method.upper()
        if is_intercepted_websocket(
            ...
            flow.metadata["codex_local_responses_websocket"] = True
        ...
        if is_cloud_auxiliary_inference_request(
            ...
            flow.metadata["codex_local_passthrough"] = True
            flow.metadata["codex_local_remote_inference"] = True
            flow.metadata["codex_local_remote_auxiliary_inference"] = True
            ...
            return
        if not is_intercepted_request(
            request.method, original_host, original_path
        ):
            flow.metadata["codex_local_passthrough"] = True
            flow.metadata["codex_local_model_catalog"] = (
        ...
        try:
            ...
            if not route_locally:
                ...
                flow.metadata["codex_local_effective_model"] = effective_model
                flow.metadata["codex_local_pro_mode"] = pro_mode
                self._audit_cloud_http_request(
                    flow,
        ...
        flow.metadata["codex_local_request_digest"] = request_digest
        ...
        if replay is not None:
            ...
            self._record(
                ...
                **replay_savings_fields(_usage_from_replay(content_type, body)),
            )
            return
        ...
        if broken_rules:
            ...
            return
        if not compaction_request and not self._preflight_local_upstream():
            # Fail fast before this turn costs inference: the local server is
            # unreachable, so forwarding would burn a full 60-200s turn for a
            # request the upstream can never answer.
            flow.response = http.Response.make(
                503,
                json.dumps(
                    {
                        "error": {
                            "message": (
                                "local model server is not reachable; refusing "
                                "this turn before it costs inference"
                            ),
                            "type": "server_error",
                        }
                    }
            ...
            return
        flow.metadata["codex_local_intercepted"] = True
        flow.metadata["codex_local_requested_model"] = requested_model
        ...
        flow.metadata["codex_local_compaction_request"] = compaction_request
        if compaction_request:
            # Kept so a failed compaction can still be summarized from the real
            # conversation instead of a placeholder.
            flow.metadata["codex_local_compaction_payload"] = payload
        flow.metadata["codex_local_client_stream"] = client_stream
        flow.metadata["codex_local_original_host"] = original_host
        flow.metadata["codex_local_original_path"] = safe_path
        ...
        request.headers["User-Agent"] = "codex-agent-harness-interceptor/0.1"
        if self.api_key and self.auth_header:
    ...
    def _native_tool_streaming_enabled(self) -> bool:
        override = os.environ.get("CODEX_LOCAL_NATIVE_TOOL_STREAMING")
    ...
    async def _run_prefix_prefill(
        ...
        *,
        ...
        def worker() -> tuple[int, int, dict[str, Any]]:
            headers = self._local_headers(
                "codex-agent-harness-prefix-prefill/0.2"
            )
            first_byte_ms = 0
            with self._pool().stream(
                body,
    ...
    def websocket_start(self, flow: http.HTTPFlow) -> None:
        if not flow.metadata.get("codex_local_responses_websocket"):
            return
        self.websocket_states[flow.id] = self._new_websocket_state()
        self._record(
    ...
    def websocket_message(self, flow: http.HTTPFlow) -> None:
        if not flow.metadata.get("codex_local_responses_websocket") or flow.websocket is None:
            return
        ...
        try:
            ...
            if replayed is not None:
                for event in replayed:
                    self._inject_websocket_event(flow, event)
                state.remember(expanded, replayed, route_local=True)
                replay_usage = next(
                    (
                        usage
                        for event in reversed(replayed)
                        if (usage := usage_from_response_event(event)) is not None
                    ),
                    None,
                )
                self._record(
                    ...
                    status=200,
                    duration_ms=round((time.monotonic() - started) * 1000),
                    **replay_savings_fields(replay_usage),
                )
                return
            if self._local_failures_exhausted(request_digest):
                self._inject_websocket_event(
            ...
            self._record_performance_warnings(
                ...
                cache_hit_percent=usage_fields(
    ...
    def _serve_synthetic_compaction(
        ...
        *,
        ...
        for event in events:
            self._inject_websocket_event(flow, event)
        response_id = state.remember(payload, events, route_local=True)
        self._record(
    ...
    async def _local_sse_events(
        ...
        *,
        ...
        def worker() -> None:
            headers = self._local_headers(
                "codex-agent-harness-websocket-bridge/0.3"
            )
            body = (
                request_body
            ...
            for attempt in range(1, LOCAL_TRANSIENT_RETRY_ATTEMPTS + 1):
                ...
                try:
                    ...
                    if (
                        ...
                        and _is_transient_upstream_exception(exc)
                    ):
                        loop.call_soon_threadsafe(
                            queue.put_nowait,
                            (
        ...
        while True:
            ...
            if kind == "first":
                ...
                if stream_directly:
                    emit_payloads(
                        decoder.feed(sanitizer.feed(normalizer.feed(value)))
                    )
                else:
                    raw.extend(value)
            elif kind == "error":
        ...
        if stream_directly:
            ...
            body = bytes(raw)
            if emittable_tools:
                body, textual_repair = adapt_textual_tool_call_sse(body, emittable_tools)
    ...
    def responseheaders(self, flow: http.HTTPFlow) -> None:
        if flow.metadata.get("codex_local_remote_inference") and flow.response is not None:
            self._audit_cloud_http_response_headers(flow)
        if not flow.metadata.get("codex_local_intercepted") or flow.response is None:
            return
        routed_at = flow.metadata.get("codex_local_local_routed_at")
        if isinstance(routed_at, (int, float)):
            flow.metadata["codex_local_route_to_first_byte_ms"] = max(
            ...
            self._record(
                ...
                local_model=self.model,
                local_server=self.local_server,
                requested_model=flow.metadata.get("codex_local_requested_model"),
                local_slot=self.local_slot,
                transport="http",
                first_byte_ms=flow.metadata["codex_local_route_to_first_byte_ms"],
            )
        content_type = flow.response.headers.get("Content-Type", "").lower()
        if "text/event-stream" not in content_type:
            return
        if flow.metadata.get("codex_local_compaction_request"):
        ...
        flow.response.headers.pop("Content-Length", None)
        normalizer = SSEToolNameNormalizer(
            flow.metadata.get("codex_local_registered_tools", []),
            flow.metadata.get("codex_local_namespace_tools", {}),
            flow.metadata.get("codex_local_client_tool_mappings", {}),
        )
        sanitizer = SSELocalOutputSanitizer()
        observer = SSEUsageObserver()
        completed_capture = _SSECompletedEventCapture()
        flow.metadata["codex_local_sse_normalizer"] = normalizer
        flow.metadata["codex_local_sse_sanitizer"] = sanitizer
        flow.metadata["codex_local_usage_observer"] = observer
        flow.metadata["codex_local_sse_completed_capture"] = completed_capture
        native_tool_streaming = self._native_tool_streaming_enabled()
        flow.metadata["codex_local_native_tool_streaming"] = native_tool_streaming
        if (
            flow.metadata.get("codex_local_emittable_tools")
            and not native_tool_streaming
        ):
            flow.metadata["codex_local_buffer_tool_stream"] = True
        else:
    ...
    def response(self, flow: http.HTTPFlow) -> None:
        if flow.metadata.get("codex_local_remote_inference") and flow.response is not None:
            ...
            self._record(
                ...
                inference_kind=(
                    "image_generation"
                    if flow.metadata.get("codex_local_remote_auxiliary_inference")
                    else "responses"
                ),
            )
            return
        if flow.metadata.get("codex_local_passthrough") and flow.response is not None:
            if flow.metadata.get("codex_local_model_catalog"):
            ...
            self._record(
                ...
                original_host=flow.metadata.get("codex_local_original_host"),
                original_path=flow.metadata.get("codex_local_original_path"),
                method=flow.metadata.get("codex_local_original_method"),
                status=flow.response.status_code,
                duration_ms=self._duration_ms(flow),
            )
            return
        if not flow.metadata.get("codex_local_intercepted") or flow.response is None:
            return
        normalizer = flow.metadata.get("codex_local_sse_normalizer")
        if normalizer is not None:
            if flow.metadata.get("codex_local_buffer_tool_stream"):
                raw = flow.response.get_content(strict=False)
                repaired, textual_call_repaired = adapt_textual_tool_call_sse(
                    raw, flow.metadata.get("codex_local_emittable_tools", [])
                )
                ...
                sanitizer = flow.metadata.get("codex_local_sse_sanitizer")
                ...
                try:
                    ...
                    if not 200 <= flow.response.status_code < 300:
                        ...
                        transformed = synthetic_compaction_response(
                            flow.metadata.get("codex_local_compaction_payload") or {},
                            self.model,
                        )
                        adapted = True
                    ...
                    if adapted:
                        flow.response.headers.pop("Content-Length", None)
                        if flow.metadata.get("codex_local_client_stream"):
        ...
        fields = {
            ...
            "local_server": self.local_server,
            "duration_ms": duration_ms,
            "first_byte_ms": flow.metadata.get("codex_local_route_to_first_byte_ms"),
            "tool_names_remapped": len(remapped),
        }
        digest = flow.metadata.get("codex_local_request_digest")
        if event == "inference_error":
            ...
            fields.update(usage_fields(self._response_usage(flow), duration_ms=duration_ms))
    ...
    def _record_local_failure(self, digest: str) -> int:
        ...
        self.local_failures = {
            key: value
            for key, value in self.local_failures.items()
            if value[0] > now
        }
    ...
    def _response_usage(self, flow: http.HTTPFlow) -> dict[str, Any] | None:
        observer = flow.metadata.get("codex_local_usage_observer")
    ...
    def _decoded_response(self, flow: http.HTTPFlow) -> Any:
        if "codex_local_decoded_response" in flow.metadata:
            return flow.metadata["codex_local_decoded_response"]
        capture = flow.metadata.get("codex_local_sse_completed_capture")
        if capture is not None:
            ...
            if completed is not None:
                flow.metadata["codex_local_decoded_response"] = completed
                return completed
        decoded: Any = None
        try:
        ...
        if raw:
            ...
            if "text/event-stream" in content_type:
                ...
                try:
                    ...
                    decoded = None
        flow.metadata["codex_local_decoded_response"] = decoded
        return decoded

    def error(self, flow: http.HTTPFlow) -> None:
        if flow.metadata.get("codex_local_remote_inference"):
            self._audit_cloud_bytes(
                ...
                direction="transport",
                transport="http",
                model=flow.metadata.get("codex_local_requested_model"),
            )
            self._record(
                "remote_inference_error",
                requested_model=flow.metadata.get("codex_local_requested_model"),
            ...
            return
        if flow.metadata.get("codex_local_passthrough"):
            self._record(
            ...
            return
        if flow.metadata.get("codex_local_intercepted"):
            normalizer = flow.metadata.get("codex_local_sse_normalizer")
    ...
    def _audit_cloud_websocket_handshake(
        ...
        *,
        ...
        if flow.metadata.get("codex_local_cloud_websocket_handshake"):
            return
        flow.metadata["codex_local_cloud_websocket_handshake"] = True
        common = {
            "flow_id": flow.id,
            "transport": "websocket",
            "model": model,
    ...
    def _audit_cloud_http_request(
        ...
        *,
        ...
        common = {
            ...
            "transport": "http",
            "method": request.method.upper(),
            "host": flow.metadata.get("codex_local_original_host", request.host),
            "path": flow.metadata.get("codex_local_original_path", request.path),
            "model": model,
    ...
    def _audit_cloud_http_response_headers(self, flow: http.HTTPFlow) -> None:
        if flow.response is None or flow.metadata.get("codex_local_cloud_response_headers"):
            return
        flow.metadata["codex_local_cloud_response_headers"] = True
        response = flow.response
        common = {
            "flow_id": flow.id,
            "direction": "openai_to_codex",
    ...
    def _audit_cloud_bytes(self, event: str, data: bytes, **fields: Any) -> None:
        ...
        try:
            if (
                fields.get("direction") == "codex_to_openai"
                and event
                in {
                    "cloud.http.request.body",
                    "cloud.http.request.body.decoded",
                    "cloud.websocket.frame",
                }
            ):
                fields["original_byte_length"] = len(data)
                fields["body_redacted"] = True
                data = redact_cloud_request_body(data)
            fields.setdefault("local_slot", self.local_slot)
            fields.setdefault("local_label", self.local_label)
    ...
    def _duration_ms(flow: http.HTTPFlow) -> int | None:
        started = flow.metadata.get("codex_local_request_started")
    ...
    def _adapt_model_catalog(self, response: http.Response) -> int:
        ...
        try:
            ...
            if should_recover and selected is not None:
    ...
    def _cache_model_catalog(self, decoded: Any) -> None:
        if not isinstance(decoded, dict) or not isinstance(decoded.get("models"), list):
            return
        path = self.status_path.parent / "codex-local-model-catalog.json"
    ...
    def _record(self, event: str, **fields: Any) -> None:
        if event in {
            ...
            fields.setdefault("served_by", "local")
            fields.setdefault("local_model", self.model)
            fields.setdefault("local_server", self.local_server)
            fields.setdefault("local_slot", self.local_slot)
            fields.setdefault("local_label", self.local_label)
        elif event in {
            "remote_inference_routed",
            ...
            fields.setdefault("served_by", "remote")
            fields.setdefault("local_slot", self.local_slot)
            fields.setdefault("local_label", self.local_label)
        payload = {
            "time": time.time(),
...
def _upstream_reachable(
    ...
    *,
    ...
    """Cheap liveness check for the local upstream before a costly turn."""
    parsed = urlsplit(url)
    if parsed.scheme not in {"http", "https"} or not parsed.hostname:
        return False
