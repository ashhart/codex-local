"""Routing rules and request/response transforms for the intercepted slot.

This module decides what Codex Local is allowed to touch. It recognises the exact
Codex Responses inference paths, rewrites only the model on a request bound for
a private endpoint, strips the OpenAI credentials before that request leaves,
and repairs the response shapes local servers get wrong. Everything it does not
recognise is left alone, which is what keeps the rest of Codex working.

It is pure standard library so that it can be imported both by the launcher and
by the mitmproxy addon, which runs under mitmproxy's own interpreter.
"""
...
SAFE_EVENT_FIELDS = frozenset(
    {
        ...
        "input_tokens",
        "replay_saved_tokens",
        "replay_saved_input_tokens",
        "replay_saved_output_tokens",
        "tool_count",
        "tool_names",
        "loop_tool_name",
        "loop_repeats",
...
def list_pi_interceptor_choices(
    *, models_path: str | Path | None = None
    ...
    for provider, definition in providers.items():
        if not isinstance(provider, str) or not provider or not isinstance(definition, dict):
            continue
...
def format_interceptor_event(event: dict[str, Any]) -> str | None:
    ...
    if event_name == "prefix_prefill_completed":
        ...
        reused = " · reused connection" if event.get("connection_reused") is True else ""
        return f"[{clock}] LOCAL ✓ Codex prefix cached{duration_text}{first_text}{reused}"
    ...
    if event_name == "model_idle_unloaded":
        idle = event.get("idle_seconds")
        idle_text = f" after {idle:g}s idle" if isinstance(idle, (int, float)) else ""
        return f"[{clock}] LOCAL ○ {model} unloaded{idle_text}"
    if event_name == "model_idle_unload_failed":
        return f"[{clock}] LOCAL ! {model} idle unload failed"
    ...
    if event_name == "doom_loop_detected":
        ...
        return (
            f"[{clock}] LOOP {action} · {name} x{event.get('loop_repeats')}{stuck}"
        )
    ...
    if event_name == "inference_routed":
        ...
        compaction = " compaction=1" if event.get("is_compaction_request") is True else ""
    ...
    if event_name in {"inference_completed", "inference_stream_closed"}:
        ...
        visible = (
            f" visible={first_visible}ms"
            if isinstance(first_visible, int)
            else ""
        )
        suffix = " stream closed cleanly" if event_name == "inference_stream_closed" else ""
        connection = (
            " reused=1"
...
def should_route_responses_locally(
    ...
    *,
    ...
    compaction_owner_local: bool | None = None,
) -> bool:
    """Route every request in legacy mode, or only the selected Codex slot.

    Compaction follows the owning task's most recent ordinary inference route.
    ...
    if requested_model == local_slot:
        return True
    if local_compaction and isinstance(payload, dict):
        compaction = is_compaction_request(payload) or is_compaction_trigger_request(
            payload
        )
        if compaction:
            # Codex can name its maintenance/prewarm request after a hosted
            # model even when the task itself uses the emulated local slot.
...
def select_lowest_visible_codex_model(payload: Any) -> dict[str, str] | None:
    ...
    return {
        "slug": model["slug"],
        "display_name": display if isinstance(display, str) and display else model["slug"],
    }
...
# Opening words of the summarization instruction appended to a compaction turn.
COMPACTION_TASK_MARKER = "STATE COMPACTION TASK"


def is_compaction_turn(payload: dict[str, Any]) -> bool:
    """Detect the summarization turn this module builds for compaction.

    The instruction is appended last, so only the tail is scanned.
    """
    inputs = payload.get("input")
    if isinstance(inputs, str):
        return COMPACTION_TASK_MARKER in inputs
    if not isinstance(inputs, list):
        return False
    for item in inputs[-3:]:
        if COMPACTION_TASK_MARKER in json.dumps(item, default=str):
            return True
    return False
...
def _context_item_to_developer_message_from_value(item: Any) -> dict[str, Any]:
    if isinstance(item, dict):
        ...
        item_type_text = item_type if isinstance(item_type, str) and item_type else "unknown"
...
def summarize_responses_request(payload: dict[str, Any], raw_bytes: int) -> dict[str, Any]:
    summary: dict[str, Any] = {
        ...
        "has_previous_response_id": isinstance(payload.get("previous_response_id"), str),
    ...
    if isinstance(metadata, dict):
        for key, value in metadata.items():
            ...
            if (
                ...
                and _looks_like_compaction_marker(value)
            ):
                return True
...
def synthetic_compaction_response(payload: dict[str, Any], model: str) -> dict[str, Any]:
...
def normalize_legacy_compaction_item_ids(value: Any) -> tuple[Any, int]:
    """Repair compaction IDs emitted by earlier releases.

    Codex accepts compaction item IDs with the ``cmp.`` prefix. Earlier
    ...
    def visit(node: Any) -> tuple[Any, int]:
        if isinstance(node, list):
            ...
            for item in node:
                ...
                normalized_list.append(repaired)
                count += item_count
        ...
        for key, item in node.items():
            repaired, item_count = visit(item)
            normalized_dict[key] = repaired
            count += item_count
        item_id = normalized_dict.get("id")
        if (
            normalized_dict.get("type") == "compaction"
            and isinstance(item_id, str)
            and item_id.startswith("ci_local_")
        ):
            normalized_dict["id"] = "cmp." + item_id.removeprefix("ci_local_")
            count += 1
    ...
    def visit(node: Any) -> tuple[Any, int, bool]:
        if isinstance(node, list):
            ...
            for item in node:
                ...
                if not drop:
                    normalized_list.append(repaired)
            return normalized_list, count, False
        if not isinstance(node, dict):
            return node, 0, False
        ...
        for key, item in node.items():
            ...
            if not drop:
                normalized_dict[key] = repaired
        return normalized_dict, count, False
    ...
    transformed["output"] = [
        {
            ...
            "created_by": "codex-agent-harness",
        }
    ]
...
def non_array_content_item_count(value: Any) -> int:
    ...
    if isinstance(value, dict):
        return 1 if "content" in value and not isinstance(value.get("content"), list) else 0
    return 0
...
def is_readable_compaction_summary(value: Any) -> bool:
    ...
    if sample.startswith("gAAAAA"):  # Fernet, as used by hosted compaction
        return False
    if len(sample) > 120 and not any(character.isspace() for character in sample):
        return False
    return True
        ...
        if (
            ...
            if matches:
                ...
                for extra_index, extra in matches[1:]:
                    repaired_items.append(
                        _recovered_history_message(
                            f"duplicate tool result {call_id}", _tool_output_value(extra)
                        )
                    )
...
def response_is_empty(value: Any) -> bool:
    ...
    One recorded session produced 18 empty assistant messages; Codex Local
...
def opaque_compaction_item_count(value: Any) -> int:
    ...
    if value.get("type") == "compaction":
        return 0 if is_readable_compaction_summary(value.get("encrypted_content")) else 1
...
def _remove_compaction_trigger_items(value: Any) -> Any:
    ...
    if value.get("type") == "compaction_trigger":
        return None
    return {
        key: _remove_compaction_trigger_items(item)
        for key, item in value.items()
    }
...
def _contains_compaction_prompt_marker(value: Any) -> bool:
    for text in _iter_strings(value):
        if _looks_like_compaction_marker(text):
            return True
    return False
...
def _synthetic_compaction_summary(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "Automatic startup compaction completed locally by Codex Local.",
...
def _recent_user_texts(value: Any, *, limit: int = 5, max_chars: int = 500) -> list[str]:
    messages: list[str] = []
    ...
    def visit(node: Any) -> Any:
        ...
        if updated.get("type") == "function_call" and isinstance(
            ...
            if namespace_match is not None:
                ...
                remapped.append(
                    {"from": original, "to": f"{namespace}::{normalized}"}
                )
            else:
...
def sanitize_local_response_items(
    value: Any, *, copy: bool = True
) -> tuple[Any, int]:
...
def interceptor_status(
    *, runtime_dir: str | Path | None = None
    ...
    if session_path.stat().st_size > 65_536:
        raise ValueError(f"interceptor session receipt is unexpectedly large: {session_path}")
    ...
    if status_path.is_file():
        ...
        if status_path.stat().st_size > 32 * 1024 * 1024:
            raise ValueError(f"interceptor status log is unexpectedly large: {status_path}")
    ...
    # A plugin or MCP tool arrives namespaced; Codex's own built-ins do not.
    # Seeing one proves plugin tool schemas still reach the model through the
    # proxy, which is the whole point of not touching Codex configuration.
    plugin_tool_advertised = any(
        TOOL_NAME_SEPARATORS.search(name) for name in registered_tools
    )
    ...
    plugin_tool_used = attestation.get("plugin_tool_used") is True
    ...
    automated_gates = {
        ...
        "plugin_tool_advertised": plugin_tool_advertised,
    ...
    return {
        ...
        "desktop_acceptance": {
            **automated_gates,
            ...
            "projects_sidebar": "verified_visible" if projects_visible else "pending_live_app_check",
            "automations": "verified_visible" if automations_visible else "pending_live_app_check",
            "plugin_tools": "verified_used" if plugin_tool_used else "pending_live_app_check",
            "computer_use": "verified_used" if computer_use_used else "pending_live_app_check",
            "local_model_response": "verified_visible" if local_response_visible else "pending_live_app_check",
            "complete": complete,
        },
        "credentials_returned": False,
    }
...
def _require_private_receipt(runtime: Path, path: Path) -> None:
    ...
    for candidate in (runtime, path):
        ...
        if hasattr(os, "getuid") and stat.st_uid != os.getuid():
            raise PermissionError(f"interceptor receipt is owned by another user: {candidate}")
